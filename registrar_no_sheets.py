...
# Reconstrói o JSON de credenciais a partir da variável de ambiente
...
def registrar_mensagem(numero, mensagem, nome):
    print("📥 Função registrar_mensagem chamada.")
    agora = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    sheet.append_row([agora, numero, nome, mensagem])
    print(f"✅ Registrado: {numero} ({nome}) - {mensagem}")
